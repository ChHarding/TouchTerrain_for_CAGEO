...
def run_get_zipped_tiles(overwrite_args, testname):
    ...
    # If Earth Engine isn't authenticated in this environment, skip EE tests
    try:
        ee.Initialize()
    except Exception as e:
        # Use unittest.SkipTest so unittest/pytest will mark these as skipped instead of failing
        raise unittest.SkipTest(
            f"Earth Engine not authenticated or Initialize failed: {e}"
        )
    ...
    folder = (
        getcwd() + sep + "test" + sep + args["zip_file_name"]
    )  # unzip into his folder inside test
...
class MyTests(unittest.TestCase):
    ...
    def test_EE_bottom_image_masked(self):
        ...
        run_get_zipped_tiles(
            args,
            "EE single tile with bottom relief, masked (should ignore bottom image!)",
        )
    ...
    def test_EE_gpx_path(self):
        ...
        run_get_zipped_tiles(
            args, "EE with GPX path, using gpx files in stuff gpx-test"
        )
    ...
    def test_local_resampled(self):
        ...
        run_get_zipped_tiles(
            args, "single tile from local geotiff, resampled to 1 mm printres"
        )
    ...
    def test_local_source_resolution(self):
        ...
        run_get_zipped_tiles(
            args, "single tile from local geotiff at source resolution"
        )
