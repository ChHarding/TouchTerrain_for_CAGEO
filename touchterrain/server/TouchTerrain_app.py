...
def verify_recaptcha(token):
    ...
    with open(RECAPTCHA_V3_LOG_FILE, "a") as f:
        f.write(
            f"{result.get('challenge_ts', '')}, {result.get('success', False)}, "
            f"{result.get('score', 0)}, {app.config['score_threshold']}, {result.get('hostname', '')}\n"
        )

    return (
        result.get("success", False)
        and result.get("score", 0) > app.config["score_threshold"]
    )
...
def intro_page():
    if request.method == "POST":
        ...
        if token and verify_recaptcha(token):
            ...
            session["recaptcha_verified"] = (
                False  # default to False, set to True if recaptcha is verified
            )
...
def main_page():
    ...
    if args["DEM_name"] in (
        ...
        elev = elev.mosaic().setDefaultProjection(
            proj
    ...
    mapid = hs.getMapId(
        {"gamma": gamma}
    )  # request map from EE, transparency will be set in JS
...
def preview(zip_file):
    ...
    for i, f in enumerate(stl_files):
        url = url_for(
            "serve_stl", job_id=job_id, filename=f
        )  # route to call with its 2 args
        models.append(
            {"id": i + 1, "filename": url, "rotationx": -0.35, "display": "flat"}
        )
...
def export():
    ...
    os.system(
        "tmpwatch --mtime 6h {} {} {}".format(
            DOWNLOADS_FOLDER, PREVIEWS_FOLDER, TMP_FOLDER
        )
    )
    ...
    def preflight_generator():
        ...
        server = (
            o.scheme + "://" + o.netloc
        )  # was: hostname e.g. https://touchterrain.geol.iastate.edu

        URL_query_str = server + make_current_URL(
            query_list
        )  # make_current_URL return will start with main? so it doesn't go to the splash screen
        ...
        html += make_GA_script(
            header
        )  # <head> with script that inits GA with my tracking id a
        ...
        timeout_msg += (
            "Your only option is to run the job again and hope for better luck.<br>"
        )
        ...
        html += (
            """<body onerror="document.getElementById('error').innerHTML=\'"""
            + "onerror: "
            + timeout_msg
            + """'"\n"""
        )
        ...
        html += (
            "Once the animation stops, you can preview and download your file.</h2>\n"
        )
            ...
            if kml_file.filename != "":  # '' happens when kml file was invalidated
                ...
                if kml_file.filename[-4:] == ".kmz":
                    ...
                    try:
                        ...
                        html += (
                            "Warning: "
                            + kml_file.filename
                            + " is not a valid kmz polygon file! (falling back to area selection box.)\n"
                        )
                ...
                try:
                    ...
                    html += (
                        "Warning: "
                        + kml_file.filename
                        + " is not a valid kml polygon file! (falling back to area selection box.)\n"
                    )
                else:
                    if (
                        msg != None
                        ...
                        if coords == None:  # got nothing good
                            html += (
                                "Warning: "
                                + kml_file.filename
                                + " contained neither polygon nor line, falling back to area selection box.<br>"
                            )
                        else:
                            html += (
                                "Warning: Using line with "
                                + str(len(coords))
                                + " points in "
                                + kml_file.filename
                                + " as no polygon was found.<br>"
                            )
                            geojson_polygon = Polygon([coords])
                    else:  # got polygon
                        geojson_polygon = Polygon(
                            [coords]
                        )  # coords must be [0], [1] etc. would be holes
                        html += (
                            "Using polygon from kml file "
                            + kml_file.filename
                            + " with "
                            + str(len(coords))
                            + " points.<br>"
        ...
        if pr > 0:  # print res given by user (width and height are in mm)
            ...
            tot_pix = int(
                (pix_per_tile * num_total_tiles) / div_by
            )  # total pixels to print
            print(
                "total requested pixels to print",
                tot_pix,
                ", max is",
                MAX_CELLS_PERMITED,
                file=sys.stderr,
            ...
            cell_width_arcsecs = {
                ...
                "MERIT/DEM/v1_0_3": 3,
                "USGS/GMTED2010": 7.5,
                "CPOM/CryoSat2/ANTARCTICA_DEM": 30,
                "NOAA/NGDC/ETOPO1": 60,
                "USGS/GTOPO30": 30,
                "USGS/SRTMGL1_003": 1,
                "JAXA/ALOS/AW3D30/V3_2": 1,
                "NRCan/CDEM": 0.75,
            ...
            print(
                "total requested pixels to print at a source resolution of",
                round(cwas, 2),
                "arc secs is ",
                tot_pix,
                ", max is",
        ...
        if tot_pix > MAX_CELLS_PERMITED:
            ...
            html += (
                '\nIf that happens, go <a href="'
                + URL_query_str
                + '">'
                + " back to the main page </a> to make adjustments."
            )
        ...
        if totalsize < 0:
            ...
            if args["fileformat"] in ("STLa", "STLb"):
                html += (
                    '<br><form action="'
                    + url_for("preview", zip_file=zip_file)
            ...
            html += (
                "   Size: %.2f Mb   (All files will be deleted in 6 hrs.)<br>\n"
                % totalsize
            )
            html += "</form>\n"
