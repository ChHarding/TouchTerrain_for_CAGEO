...
try:
    ...
    # Only warn once to avoid flooding logs in multiprocessing scenarios
    if not os.environ.get("TT_EE_WARNING_SHOWN"):
        logging.warning(
            f"EE init() error (with {EE_CREDS}) {e} (This is OK if you don't use earthengine anyway!)"
        )
        os.environ["TT_EE_WARNING_SHOWN"] = "1"
else:
    if not os.environ.get("TT_EE_SUCCESS_SHOWN"):
        logging.info(f"EE init() worked with {EE_CREDS}")
        os.environ["TT_EE_SUCCESS_SHOWN"] = "1"
...
initial_args = {
    ...
    "ntilesy": 2,
    "tilewidth": 220,  # width of each tile in mm (<- !!!!!), tile height is calculated
    ...
    "max_cells_for_memory_only": 3000
    * 3000,  # if raster has more cells, use temp_files instead of memory (slower, but can be huge)
...
def process_tile(tile_tuple):
    ...
    if tile_info["bottom_image"] != None and tile_info["no_bottom"] != None:
        ...
        bottom_raster *= (
            tile_info["base_thickness_mm"] * 0.8
    ...
    if bottom_raster is not None:
        del bottom_raster
    if tile_elev_orig_raster is not None:
        del tile_elev_orig_raster
...
def get_zipped_tiles(
    ...
    trlon=None,
    bllat=None,
    bllon=None,  # all args are keywords, so I can use just **args in calls ...
    polygon=None,
    polyURL=None,
    poly_file=None,
    importedDEM=None,
    ...
    **otherargs,
    ...
    assert fileformat in ("obj", "STLa", "STLb", "GeoTiff"), (
    ...
    if importedDEM == None:  # GEE as DEM source
        ...
        assert os.path.exists(importedDEM), (
            "Error: local DEM raster file " + importedDEM + " does not exist"
        )
        assert fileformat != "GeoTiff", (
    ...
    if only != None:
        ...
        assert (
            only[1] > 0 and only[1] <= num_tiles[1]
    ...
    if polygon != None:
        ...
        if isinstance(polygon, dict):
            ...
            ), f"Error: dict is not a GeoJSON polygon: {polygon}"

            # Extract polygon coordinates (throw away [1] which would be a doughnut hole)
            clip_poly_coords = polygon["coordinates"][
                0
            ...
            assert (
                False
            ), f"Error: coordinate format must be: [[[x,y], [x,y], ...]] not {polygon}"
    ...
    # TODO: deal with clip polygon?  Done for KML (poly_file)
        ...
        if poly_file != None and poly_file != "":
            ...
            clipped_geotiff = (
                os.path.join(folder, clipped_filename) if folder else clipped_filename
            )
    ...
    if fileformat != "GeoTiff":  # Mesh export
        ...
        if lower_leq is not None:
            ...
            offset = (
                lower_leq[1] / 1000 * print3D_scale_number
        ...
        if offset_masks_lower is not None:
            ...
            for offset_layer in offset_npim:
                offset = (
                    offset_masks_lower[count][1] / 1000 * print3D_scale_number
                )  # scale mm up to real world meters
                ...
                pr(
                    ...
                    offset_masks_lower[count][0],
                    "by",
                    offset,
                    "m, equiv. to",
                    offset_masks_lower[count][1],
                    "mm at map scale",
                )
        ...
        if bottom_elevation is not None:
            ...
            close_values = np.isclose(
                top, bottom, rtol=0, atol=0.001, equal_nan=False
            )  # bool array
        ...
        for tx in range(num_tiles[0]):
            for ty in range(num_tiles[1]):
                ...
                tile = (
                    my_tile_info,
                    tile_elev_raster,
                    tile_bot_elev_raster,
                    tile_elev_orig_raster,
    ...
    if map_img_filename != None:
        try:
            ...
            print(
                "Error removing map image" + str(map_img_filename) + " " + str(e),
                file=sys.stderr,
            )
