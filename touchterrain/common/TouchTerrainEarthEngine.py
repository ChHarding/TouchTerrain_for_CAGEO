...
try:
    ...
    logging.warning(
        f"EE init() error (with {EE_CREDS}) {e} (This is OK if you don't use earthengine anyway!)"
    )
else:
...
def make_bottom_raster_from_image(image_file_name, shape):
    ...
    print(
        "Used", image_file_name, "for bottom relief, rescaled to", scaled_size, end=" "
    )
...
def process_tile(processingTile: ProcessingTile):
    tile_info = (
        processingTile.tile_info
    )  # tile_tuple[0] # has info for this individual tile
    ...
    if isinstance(processingTile.top_raster_variants.dilated, numpy.ndarray):
        tile_elev_raster = (
            processingTile.top_raster_variants.dilated
        )  # tile_tuple[1] # the actual (top) raster
    ...
    if isinstance(processingTile.bottom_raster_variants, RasterVariants):
        if isinstance(processingTile.bottom_raster_variants.dilated, numpy.ndarray):
            tile_bottom_raster = (
                processingTile.bottom_raster_variants.dilated
            )  # tile_tuple[2] # the actual (bottom) raster (or None)
    ...
    if tile_info.config.bottom_image != None and tile_info.config.no_bottom != None:
        ...
        bottom_raster = make_bottom_raster_from_image(
            tile_info.config.bottom_image, tile_elev_raster.shape
        )
        # print "min/max:", numpy.nanmin(bottom_raster), numpy.nanmax(bottom_raster)
        bottom_raster *= (
            tile_info.config.basethick * 0.8
        )  # max relief is 80% of base thickness to still have a bit of "roof"
        print(
            "bottom image (in meters!) min/max:",
            numpy.nanmin(bottom_raster),
            numpy.nanmax(bottom_raster),
        )  # range of bottom raster
...
def resampleDEM(a, factor):
    ...
    if numpy.isnan(numpy.sum(a)):
        ...
        a = numpy.where(
            numpy.isnan(a), nanmin - 1, a
        )  # swap NaN to a bit smaller than valid min, so the interpolation works
    ...
    img = Image.fromarray(
        a
    )  # was using fromarray(a, 'F') but that affected the elevation value, which were much lower!
    # print "pre-resam", img.size
    # print "resamp 2.5 min/max", img.getextrema()
    img = img.resize(
        newsh[::-1], resample=Image.BILINEAR
    )  # x and y are swapped in numpy
    ...
    for layer in layers:
        ...
        for feature in features:
            ...
            if geom_type == "Polygon":
                coords_2d = [
                    [c3[0], c3[1]] for c3 in coords[0]
                ]  # [0] -> ignore holes, which would be in 1,2, ...
        ...
        for feature in features:
            ...
            if geom_type == "LineString":
                ...
                return (
                    coords_2d,
                    "Warning: no Polygon found, used a (closed) Line instead",
                )
...
def raster_preparation(
    ...
    top_hint: numpy.ndarray | None = None,
    bottom_thru_base: bool = False,
    bottom_floor_elev: float | None = None,
) -> bool:
    ...
    if bottom.original is None:
        ...
        if numpy.any(numpy.isnan(top_npim)):
            ...
            if top_hint is not None:
                ...
                hint_nan_to_base_mask = numpy.logical_and(
                    numpy.isnan(top.original), numpy.logical_not(numpy.isnan(top_hint))
                )
                ...
                if numpy.any(hint_nan_to_base_mask) == True:
                    ...
                    top.original[hint_nan_to_base_mask] = (
                        bottom_floor_elev  # set ALL locs within top_hint mask area with a constant value
                    )

                    top_npim = dilate_array(
                        top_npim,
                        dilation_source=numpy.full(top_npim.shape, bottom_floor_elev),
                        limit_mask=hint_nan_to_base_mask,
                    )  # dilate in top_hint mask area with a constant value
    ...
    # Difference mode
        ...
        hint_nan_to_base_mask = numpy.logical_and(
            numpy.isnan(bottom.original), numpy.logical_not(numpy.isnan(top.original))
        )
        if numpy.any(hint_nan_to_base_mask) == True:
            ...
            bot_npim[hint_nan_to_base_mask] = (
                bottom_floor_elev  # set ALL locs within top_hint mask area with a constant value
            )
        ...
        close_values = numpy.isclose(
            top_npim, bot_npim, rtol=0, atol=0.001, equal_nan=False
        )  # bool array
        ...
        if numpy.any(close_values) == True:
            ...
            top_npim = dilate_array(
                top_npim, top.original, dilation_cycles=2
            )  # dilate the NaN'd top with the original (pre NaN'd) top
        ...
        if bottom_thru_base == True:
            ...
            bot_npim = dilate_array(
                bot_npim, top.original, dilation_cycles=2
            )  # dilate the NaN'd bottom with the original (pre NaN'd) top (same as original bottom)
...
def get_zipped_tiles(user_dict: dict[str, Any]):
    ...
    assert config.fileformat in ("obj", "STLa", "STLb", "GeoTiff"), (
        "Error: unknown 3D geometry file format:"
        + config.fileformat
    ...
    assert not (config.bottom_image != None and config.no_bottom == True), (
        "Error: Can't use no_bottom=True and also want a bottom_image ("
        + config.bottom_image
        + ")"
    )
    assert not (config.bottom_image != None and config.basethick <= 0.5), (
        "Error: base thickness ("
        + str(config.basethick)
        + ") must be > 0.5 mm when using a bottom relief image"
    )

    assert not (
        config.bottom_elevation != None and config.bottom_image != None
    ), "Error: Can't use both bottom_elevation and bottom_image"
    assert not (
        config.bottom_image != None and config.top_thickness != None
    ), "Error: Can't use both bottom_image and top_thickness"
    assert not (config.bottom_elevation != None and config.no_bottom == True), (
        "Error: Can't use no_bottom=True and also want a bottom_elevation ("
        + config.bottom_elevation
        + ")"
    )
    assert not (
        config.bottom_elevation != None and config.top_thickness != None
    ), "Error: Can't use both bottom_elevation and top_thickness"

    assert not (
        config.bottom_elevation != None and config.use_geo_coords != None
    ...
    if config.offset_masks_lower != None:
        for offset_pair in config.offset_masks_lower:
            ...
            assert os.path.exists(offset_pair[0]), (
                "Error: local offset mask raster file "
                + offset_pair[0]
                + " does not exist"
            )
    ...
    if not os.path.exists(config.temp_folder):  # do we have a temp folder?
        try:
            ...
            assert False, (
                config.temp_folder + "doesn't exists but could also not be created"
            )
    ...
    if config.zip_file_name is None or len(config.zip_file_name) == 0:
        if config.DEM_name == TouchTerrainConfig().DEM_name or config.DEM_name is None:
            if config_filename is not None:
                ...
                print(
                    "No non-default DEM_name or config_path passed for determining zip_file_name"
                )
                return
        else:
    ...
    if config.only != None:
        assert (
            config.only[0] > 0 and config.only[0] <= num_tiles[0]
        ), "Error: x index of only tile out of range"
        assert (
            config.only[1] > 0 and config.only[1] <= num_tiles[1]
    ...
    if config.polygon != None:
        ...
        if isinstance(config.polygon, dict):
            ...
            ), f"Error: dict is not a GeoJSON polygon: {config.polygon}"

            # Extract polygon coordinates (throw away [1] which would be a doughnut hole)
            clip_poly_coords = config.polygon["coordinates"][
                0
            ]  # ignore holes, which would be in 1,2, ...
        elif isinstance(
            config.polygon, list
        ):  # maybe it's just the coordinates? [[60.48766, -81.597101], [60.571116, -81.598891], ...]
            clip_poly_coords = config.polygon[0]
        else:
            assert (
                False
            ), f"Error: coordinate format must be: [[[x,y], [x,y], ...]] not {config.polygon}"

        logging.info(
            "Using GeoJSON polygon for masking with "
            + str(len(clip_poly_coords))
            + " points"
        )

        # make area selection box from bounding box of polygon
        config.trlat, config.trlon, config.bllat, config.bllon = get_bounding_box(
            clip_poly_coords
        )
        ...
        if (
            len(clip_poly_coords) == 5
        ):  # is it a 5 point geemap box polygon: 4 points + overlap with first
            ...
            if (
                ...
                print(
                    "ignoring geemap box polygon, using bounding box",
                    config.trlat,
                    config.trlon,
                    config.bllat,
                    config.bllon,
    ...
    # TODO: TEST THIS!!!!!!
        ...
        if (
            matches and len(matches.groups()) == 1
        ):  # need to have exactly one group match
            file_URL = "https://docs.google.com/uc?export=download&id=" + matches.group(
                1
            )
        else:
        ...
        try:
            ...
            pr(
                "Error: GDrive kml download failed",
                e,
                " - falling back to region box",
                config.trlat,
                config.trlon,
                config.bllat,
                config.bllon,
        ...
        try:
            ...
            if (
                ...
                config.trlat, config.trlon, config.bllat, config.bllon = (
                    get_bounding_box(clip_poly_coords)
                )
    ...
    if config.importedDEM == None:
        ...
        dict_for_url = (
            {}
        )  # dict with only those args that are valid for a URL query string
        for k in (
            "DEM_name",
            "trlat",
            "trlon",
        ...
        if config.unprojected == False:
            if config.projection != None:
                ...
                config.bllat > 70
            ):  # too far north for UTM, use Arctic Polar Stereographic
                # utm_zone_str = "WGS 84 / Arctic Polar Stereographic"
                epsg = 3995
                crs_str = f"EPSG:{epsg}"
                pr(
                    "Too far north for UTM - using Arctic Polar Stereographic projection (EPSG 3995)"
                )

            elif (
                config.trlat < -55
            ):  # too far south for UTM, use Arctic Polar Stereographic
        ...
        latitude_in_m, longitude_in_m = arcDegr_in_meter(
            center[1]
        )  # returns: (latitude_in_m, longitude_in_m)
        region_size_in_degrees = [
            abs(region[0][0] - region[1][0]),
            abs(region[0][1] - region[2][1]),
        ]
        pr("lon/lat size in degrees:", region_size_in_degrees)
        region_ratio_for_degrees = region_size_in_degrees[1] / float(
            region_size_in_degrees[0]
        )
        ...
        #
        region_size_in_meters = [
            region_size_in_degrees[0] * longitude_in_m,  # 0 -> EW, width
            region_size_in_degrees[1] * latitude_in_m,
        ]  # 1 -> NS, height
        ...
        if config.tilewidth_scale != None:
            config.tilewidth = (
                region_size_in_meters[1] / config.tilewidth_scale * 1000
            )  # new tilewidth in mm
            pr(
                "Overriding tilewidth using a tilewidth_scale of 1 :",
                config.tilewidth_scale,
                ", region width is",
                region_size_in_meters[1],
                "m, new tilewidth is",
                config.tilewidth,
                "(Note that the final scale may be slighly different!)",
            )
        ...
        if print3D_resolution_mm > 0:
            ...
            num_samples_lat = print3D_width_total_mm / float(
                print3D_resolution_mm
            )  # width
            num_samples_lon = print3D_height_total_mm / float(
                print3D_resolution_mm
            )  # height
        ...
        if config.DEM_name in (
            ...
            image1 = elev.mosaic().setDefaultProjection(
                proj
            )  # must mosaic collection into single image
        else:
        ...
        reg_rect = ee.Geometry.Rectangle(
            [[config.bllon, config.bllat], [config.trlon, config.trlat]]
        )  # opposite corners
        if config.polygon == None:
            polygon_geojson = (
        ...
        if config.DEM_name == """NOAA/NGDC/ETOPO1""":
            tif = [f for f in tifl if "ice_surface" in f][
                0
            ]  # get the DEM tif that has the ice surface
        else:
        ...
        GEE_dem_filename = (
            config.temp_folder + os.sep + config.zip_file_name + "_dem.tif"
        )
        ...
        ):  # for Geotiff output, we don't need to make a numpy array, etc, just close the GDAL dem so we can move it into the zip later
            del band
        ...
        #    dem = None #  Python GDAL's way of closing/freeing the raster
            ...
            assert abs(geo_transform[1]) == abs(
                geo_transform[5]
            ...
            if config.importedGPX != None and config.importedGPX != []:
                ...
                addGPXToModel(
                    ...
                    dem,
                    config.importedGPX,
                    config.gpxPathHeight,
                    config.gpxPixelsBetweenPoints,
                    config.gpxPathThickness,
                    config.trlat,
                    config.trlon,
                    config.bllat,
                    config.bllon,
            ...
            if numpy.nanmax(npim) > 16384:
                ...
                pr("omitting cells with elevation > 16384")
            pr(
                "full (untiled) raster (height,width) ",
                npim.shape,
                npim.dtype,
                "elev. min/max:",
                numpy.nanmin(npim),
            ...
            # width/height of full 3D model (all tiles together)
            print3D_width_total_mm = (
                print3D_width_per_tile * num_tiles[0]
            )  # width => EW
            print3D_height_total_mm = (
                print3D_width_total_mm * region_ratio
            )  # height => NS
            ...
            if config.printres > 0:  # did NOT use source resolution
                ...
                pr(
                    "adjusted print res from the requested",
                    print3D_resolution_mm,
                    "mm to",
                    adjusted_print3D_resolution,
                    "mm to ensure correct model dimensions",
                )
                ...
                pr(
                    "print res for native source resolution is",
                    print3D_resolution_mm,
                    "mm",
                )
    ...
    # TODO: split the GEE and imported DEM code into separate functions
        ...
        pr(
            ...
            "x",
            num_tiles[1],
            "3D model tile(s) from",
            importedDEM_filename,
            btxt,
            "\n",
        )
        ...
        if config.poly_file != None and config.poly_file != "":
            clipped_geotiff = os.path.join(
                importedDEM_folder, "clipped_" + importedDEM_filename
            )
            ...
            try:
                ...
                pr(
                    ...
                    "with",
                    config.poly_file,
                    "failed, using unclipped geotiff. ",
                    e,
                )
            else:
                pr(
        ...
        npim = cast(numpy.ndarray, band.ReadAsArray()).astype(
            numpy.float64
        ...
        if importedDEM_interp_filename:
            ...
            interp_npim = cast(numpy.ndarray, interp_band.ReadAsArray()).astype(
                numpy.float64
            )  # top interpolation elevation values
        ...
        if config.top_elevation_hint:
            ...
            top_elevation_hint_npim = cast(
                numpy.ndarray, top_elevation_hint_band.ReadAsArray()
        ...
        if pw != ph:
            logger.warning(
                "Warning: raster cells are not square ("
                + str(pw)
                + "x"
                + str(ph)
                + ") , using"
        ...
        if (
            ...
            undef_cells = numpy.isclose(
                npim, dem_undef_val
            )  # bool with cells that are close to the GDAL undef value
            npim = numpy.where(
                undef_cells, numpy.nan, npim
            )  # replace GDAL undef values with nan
            if interp_npim is not None:
                interp_npim = numpy.where(
                    numpy.isclose(interp_npim, dem_undef_val), numpy.nan, interp_npim
                )  # replace GDAL undef

            if top_elevation_hint_npim is not None:
                top_elevation_hint_npim = numpy.where(
                    numpy.isclose(top_elevation_hint_npim, dem_undef_val),
                    numpy.nan,
                    top_elevation_hint_npim,
                )  # replace GDAL undef
        ...
        if config.bottom_elevation != None or config.top_thickness != None:
            ...
            ras_npim = ras_band.ReadAsArray().astype(
                numpy.float64
            )  # bottom elevation or thickness values as numpy array
            ras_tf = ras.GetGeoTransform()
            ras_pw, ras_ph = abs(ras_tf[1]), abs(ras_tf[5])  # pixel width and height
            if ras_pw != pw or ras_ph != ph:
                logger.warning(
                    "Warning: bottom_elevation or top_thickness raster cells are not square ("
                    + str(ras_pw)
                    + "x"
                    + str(ras_ph)
                    + ") , using"
            ...
            if dem.RasterXSize != ras.RasterXSize or dem.RasterYSize != ras.RasterYSize:
                assert (
                    False
                ), f"Error: bottom_elevation or top_thickness raster sizes ({ras.RasterXSize} x {ras.RasterYSize}) does not match (top) DEM size ({dem.RasterXSize} x {dem.RasterYSize})"

            if (
                ras_cell_size_m != cell_size_m
            ):  # do bottom/thickness cells match top cells?
                assert (
                    False
                ), f"Error: bottom_elevation or top_thickness raster cell size ({ras_cell_size_m}) does not match (top) DEM cell size ({cell_size_m})"
            ...
            if ras_proj_str != proj_str or ras_datum_str != datum_str:
                assert (
                    False
                ), f"Error: bottom_elevation or top_thickness raster projection ({ras_proj_str}) or datum ({ras_datum_str}) does not match (top) DEM projection ({proj_str}) or datum ({datum_str})"
            ...
            if (
                ...
                )  # bool with cells that are close to the undef value
                ras_npim = numpy.where(
                    undef_cells, numpy.nan, ras_npim
        ...
        if config.ignore_leq != None:
            ...
            if interp_npim is not None:
                interp_npim = numpy.where(
                    interp_npim <= config.ignore_leq, numpy.nan, interp_npim
                )
        ...
        if config.tilewidth_scale != None:
            ...
            pr(
                ...
                "m, new tilewidth is",
                config.tilewidth,
                "mm. (Note that the final scale may be slighly different!)",
            )
        ...
        if config.tileScale is not None:  # use config.tileScale
            ...
            print3D_width_per_tile, print3D_height_per_tile = get_print3D_dimensions(
                dem=dem, tile_scale=config.tileScale
            )
        ...
        source_print3D_resolution = (print3D_width_per_tile * config.ntilesx) / float(
            npim.shape[1]
        )
        pr(
        ...
        if (
            config.printres <= 0
        ):  # use of source resolution was requested (typically set as -1)
            pr(
                "no resampling, using source resolution of ",
                source_print3D_resolution,
                "mm for a total model width of",
                print3D_width_total_mm,
                "mm",
            )
            if source_print3D_resolution < 0.2 and config.fileformat != "GeoTiff":
                pr(
                    "Warning: this print resolution of",
                    source_print3D_resolution,
                    "mm is pretty small for a typical nozzle size of 0.4 mm. You might want to use a printres that's just a bit smaller than your nozzle size ...",
                )
            ...
            if scale_factor < 1.0:
                pr(
                    "Warning: will re-sample to a resolution finer than the original source raster. Consider instead a value for printres >",
                    source_print3D_resolution,
                )
            ...
            if config.bottom_elevation != None:
                pr(
                    ...
                    cell_size_m,
                    "m ",
                    numpy.nanmin(bot_npim),
                    "-",
                    numpy.nanmax(bot_npim),
                    "m",
                )
            ...
            for index, offset_layer in enumerate(offset_npim):
                pr(
                    "re-sampling offset layer",
                    index,
                    ":\n ",
                    offset_layer.shape[::-1],
                    source_print3D_resolution,
                    "mm ",
                    cell_size_m,
                    "m ",
                    numpy.nanmin(offset_layer),
                    "-",
                    numpy.nanmax(offset_layer),
                    "m",
                )
            ...
            if config.tileScale is not None:
                print3D_width_per_tile, print3D_height_per_tile = (
                    get_print3D_dimensions(dem=dem, tile_scale=config.tileScale)
                )
            ...
            if adjusted_print3D_resolution != print3D_resolution_mm:
                pr(
                    ...
                    "to",
                    adjusted_print3D_resolution,
                    "to ensure correct model dimensions",
                )
                print3D_resolution_mm = adjusted_print3D_resolution
            else:
    ...
    if config.fileformat != "GeoTiff":  # Mesh export
        ...
        if config.importedDEM == None:
            config.DEM_name = config.DEM_name.replace(
                "/", "-"
            )  # replace / with - to be safe
        else:
        ...
        if remx > 0 or remy > 0:
            pr(
                f"Cropping for nice fit of {num_tiles[0]} (width) x {num_tiles[1]} (height) tiles, removing: {remx} columns, {remy} rows"
            )
            ...
            for index, offset_layer in enumerate(offset_npim):
                offset_npim[index] = offset_layer[
                    0 : offset_layer.shape[0] - remy, 0 : offset_layer.shape[1] - remx
                ]

            # adjust tile width and height to reflect the smaller, cropped raster
            ratio = old_shape[0] / float(npim.shape[0]), old_shape[1] / float(
                npim.shape[1]
            )
            pr(
                " cropping changed physical size from",
                print3D_width_per_tile,
                "mm x",
                print3D_height_per_tile,
                "mm",
            )
        ...
        print3D_scale_number = (npim.shape[1] * cell_size_m) / (
            print3D_width_total_mm / 1000.0
        )  # map scale ratio (mm -> m)
        ...
        if config.zscale < 0:
            unscaled_elev_range_m = numpy.nanmax(npim) - numpy.nanmin(
                npim
            )  # range at 1 x scale
            scaled_elev_range_m = (
                unscaled_elev_range_m / print3D_scale_number
            )  # convert range from real m to model/map m
            pos_zscale = -config.zscale
            requested_elev_range_m = (
                -config.zscale / 1000
            )  # requested range as m (given as mm)
            config.zscale = (
                requested_elev_range_m / scaled_elev_range_m
            )  # z-scale needed to get to a model with the requested range
            pr(
                "From requested model height of",
                pos_zscale,
                "mm, calculated a z-scale of",
                config.zscale,
            )
        ...
        if config.lower_leq is not None:
            ...
            offset = (
                config.lower_leq[1] / 1000 * print3D_scale_number
            )  # scale mm up to real world meters
        ...
        if config.offset_masks_lower is not None:
            ...
            for offset_layer in offset_npim:
                offset = (
                    config.offset_masks_lower[count][1] / 1000 * print3D_scale_number
                )  # scale mm up to real world meters
                ...
                pr(
                    "Offset masked elevations by raising all non masked areas of",
                    config.offset_masks_lower[count][0],
                    "by",
                    offset,
                    "m, equiv. to",
                    config.offset_masks_lower[count][1],
                    "mm at map scale",
                )
        ...
        if config.fill_holes is not None and (
            config.fill_holes[0] > 0 or config.fill_holes[0] == -1
        ):
            npim = fillHoles(
                npim, num_iters=config.fill_holes[0], num_neighbors=config.fill_holes[1]
            )
        ...
        bottom_raster_variants = RasterVariants(
            original=None, nan_close=None, dilated=None, edge_interpolation=None
        )
        ...
        if config.edge_fit_polygon_file:
            ...
            top_raster_variants.clipping_intersection_geometry = numpy.empty(
                top_raster_variants.original.shape, dtype=object
            )
                ...
                for j in range(0, top_raster_variants.original.shape[0]):  # Y
                    for i in range(0, top_raster_variants.original.shape[1]):  # X
                        ...
                        quadPrint2DCoords = arrayCellCoordToQuadPrint2DCoords(
                            array_coord_2D=(i, j),
                            cell_size=print3D_resolution_mm,
                            tile_y_shape=npim.shape[0],
                        )
                        ...
                        if isinstance(clippingPrint2DPoly, shapely.Polygon):
                            if clippingPrint2DPoly.disjoint(quadPrint2DPoly):
                                ...
                                top_raster_variants.set_location_in_variants(
                                    location=(j, i),
                                    new_value=numpy.nan,
                                    set_edge_interpolation=False,
                                )
                                ...
                                intersection_geometry = (
                                    clippingPrint2DPoly.intersection(quadPrint2DPoly)
                                )
                                ...
                                flat_intersection_geometries = flatten_geometries(
                                    [intersection_geometry]
                                )
                                if len(flat_intersection_geometries) > 0:
                                    top_raster_variants.clipping_intersection_geometry[
                                        j
                                    ][i] = flat_intersection_geometries
                        else:
        ...
        if (
            raster_preparation(
                top=top_raster_variants,
                bottom=bottom_raster_variants,
                top_hint=top_elevation_hint_npim,
            ...
            is False
            or top_raster_variants.dilated is None
        ):
            return
        ...
        if (
            config.min_elev != None
        ):  # user-given minimum elevation (via min_elev argument)
            if bottom_raster_variants.dilated is not None:  # have a bottom elevation
                min_bottom_elev = numpy.nanmin(
                    bottom_raster_variants.dilated
                )  # (actual minimum elev for bottom raster (and presumably all) tiles)
        ...
        print(
            f"elev min/max : {config.min_elev:.2f} to {numpy.nanmax(top_raster_variants.dilated):.2f}"
        )
        if bottom_raster_variants.dilated is not None:
            print(
                f"bottom elev min/max : {numpy.nanmin(bottom_raster_variants.dilated):.2f} to {numpy.nanmax(bottom_raster_variants.dilated):.2f}"
            )

        #
        # plot DEM and histogram, save as png
        #
        plot_file_name = plot_DEM_histogram(
            top_raster_variants.dilated, config.DEM_name, config.temp_folder
        )
        ...
        cells_per_tile_x = int(
            top_raster_variants.dilated.shape[1] / num_tiles[0]
        )  # tile size in pixels
        ...
        tile_info.full_raster_height, tile_info.full_raster_width = (
            top_raster_variants.dilated.shape
        )
        ...
        tile_list: list[ProcessingTile] = (
            []
        )  # list of tiles to be processed via multiprocessing.map()
        for tx in range(num_tiles[0]):
            for ty in range(num_tiles[1]):
                ...
                tile_top_raster_variants = (
                    top_raster_variants.create_tile_raster_variants(
                        start_y, end_y, start_x, end_x
                    )
                )

                tile_bottom_raster_variants = (
                    bottom_raster_variants.create_tile_raster_variants(
                        start_y, end_y, start_x, end_x
                    )
                )
                ...
                my_tile_info = copy.copy(
                    tile_info
                )  # make a copy of the global info, so we can store tile specific info in during processing

                # if raster is too large, use temp files to create the tile STL/obj files
                if (
                    tile_info.full_raster_height * tile_info.full_raster_width
                    > config.max_cells_for_memory_only
                ):
                ...
                tile = ProcessingTile(
                    tile_info=my_tile_info,
                    top=tile_top_raster_variants,
                    bottom=tile_bottom_raster_variants,
                )
                ...
                if process_only == None:  # "only" parameter was not given
                    ...
                    if (
                        process_only[0] == tile_info.tile_no_x
                        and process_only[1] == tile_info.tile_no_y
                    ):
        ...
        if (
            ...
            logger.debug(
                "tempfile or memory? number of pixels:"
                + str(tile_info.full_raster_height * tile_info.full_raster_width)
                + ">"
                + str(config.max_cells_for_memory_only)
                + " => using temp file"
            )
        ...
        if (
            ...
            or config.CPU_cores_to_use == 1
            or config.CPU_cores_to_use == None
        ):
            ...
            for i, t in enumerate(tile_list):
                ...
                pt = process_tile(
                    t
                )  # pt is a tuple: [0]: updated tile info, [1]: grid object (or None)
                if pt[1] is not None:  # if we got a grid object and not None
                    ...
                    try:  # delete temp file b/c it's only a STLb header from a tile with no elevations
                        ...
                        logger.error(
                            "Error removing" + str(pt[0]["temp_file"]) + " " + str(e)
                        )
        ...
        # use multi-core processing
            ...
            pr(
                ...
                + num_core_str
                + " cores (no logging info available while processing)  ..."
            )
            ...
            pool = mp.Pool(
                processes=num_cores, maxtasksperchild=1
            )  # processes=None means use all available cores
        ...
        for p in processed_list:
            ...
            tile_label = (
                f"_tile_{tile_info.tile_no_x}_{tile_info.tile_no_y}"
            ...
            logger.debug(
                "adding tile %d %d, total size is %d"
                % (tile_info.tile_no_x, tile_info.tile_no_y, total_size)
            )
            ...
            pr(
                ...
                tile_info.tile_no_y,
                ": height: ",
                tile_info.config.min_elev,
                "-",
                tile_info.max_elev,
                "mm",
                ", file size:",
                round(tile_info.file_size),
                "Mb",
            )
        ...
        # make k3d render
        if config.kd3_render == True and (
            config.fileformat == "STLa" or config.fileformat == "STLb"
        ):
            if tile_info.temp_file != None:
                html_file = k3d_render_to_html(
                    stl_list, config.temp_folder, buffer=False
                )
            else:
                html_file = k3d_render_to_html(
                    stl_list, config.temp_folder, buffer=True
                )
    ...
    if config.importedDEM == None:
        ...
        if config.fileformat != "GeoTiff":  # for now only for mesh output
            ...
            pr(
                "added histogram of elevation values as "
                + DEM_title
                + "_DEMandHistogram.png"
            )
    ...
    if config.importedDEM == None:
        try:
            ...
            print(
                "Error removing " + str(GEE_dem_filename) + " " + str(e),
                file=sys.stderr,
            )
    ...
    try:
        ...
        print(
            "Error removing logfile " + str(log_file_name) + " " + str(e),
            file=sys.stderr,
        )
    ...
    if config.map_img_filename != None:
        try:
            ...
            print(
                "Error removing map image"
                + str(config.map_img_filename)
                + " "
                + str(e),
                file=sys.stderr,
            )
    ...
    if config.fileformat != "GeoTiff":
        try:
            ...
            print(
                "Error removing plot_with_histogram.png "
                + str(plot_file_name)
                + " "
                + str(e),
                file=sys.stderr,
