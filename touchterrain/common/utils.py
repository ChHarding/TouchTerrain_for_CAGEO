...
from touchterrain.common.calculate_ticks import (
    calculate_ticks,  # calculate nice ticks for elevation visualization
)
...
def clean_up_diags(ras: numpy.ndarray):
    ...
    while True:
        ...
        mask = numpy.invert(numpy.isnan(ras)).astype(
            numpy.int8
        )  # int8 is the cheapest possible int in numpy
        ...
        p = numpy.array([[1, 0], [0, 1]])  # pattern to detect with hit-or-miss

        # print(mask, "input raster")
        hm = ndimage.binary_hit_or_miss(mask, structure1=p, origin1=-1).astype(
            numpy.int8
        ...
        mask = (
            mask - hm
        )  # subtract hit-or-miss results to change the location of upper left pattern cell from in the mask 1 to 0
...
def fillHoles(raster, num_iters=-1, num_neighbors=7, NaN_are_holes=False):
    ...
    if num_neighbors < 0 or num_neighbors > 9:
        print(
            "fill_holes neighbor threshold must be in range of [1,9] for 3x3 footprint. Defaulting to 7."
        )
        num_neighbors = 7
    ...
    while (holesFilledLastRound > 0) and (num_iters == -1 or num_iters > 0):
        ...
        raster = ndimage.generic_filter(
            raster, checkForAndFillHole, footprint=footprint, mode="nearest"
        )
        # print(raster)
...
def arrayCellCoordToGeoCoord(
    array_coord_2D: tuple[float, float], geo_transform: tuple
) -> tuple[float, float]:
...
def arrayCellCoordToPrint2DCoord(
    array_coord_2D: tuple[float, float], cell_size: float, tile_y_shape: int
) -> tuple[float, float]:
...
def arrayCellCoordToQuadPrint2DCoords(
    array_coord_2D: tuple[float, float], cell_size: float, tile_y_shape: int
    ...
    """
    NW = arrayCellCoordToPrint2DCoord(
        array_coord_2D=(array_coord_2D[0] - 0.5, array_coord_2D[1] - 0.5),
        cell_size=cell_size,
        tile_y_shape=tile_y_shape,
    )
    SW = arrayCellCoordToPrint2DCoord(
        array_coord_2D=(array_coord_2D[0] - 0.5, array_coord_2D[1] + 0.5),
        cell_size=cell_size,
        tile_y_shape=tile_y_shape,
    )
    SE = arrayCellCoordToPrint2DCoord(
        array_coord_2D=(array_coord_2D[0] + 0.5, array_coord_2D[1] + 0.5),
        cell_size=cell_size,
        tile_y_shape=tile_y_shape,
    )
    NE = arrayCellCoordToPrint2DCoord(
        array_coord_2D=(array_coord_2D[0] + 0.5, array_coord_2D[1] - 0.5),
        cell_size=cell_size,
        tile_y_shape=tile_y_shape,
    )
...
def geoCoordToPrint2DCoord(
    geoCoord2D: shapely.Geometry | tuple[float, float],
    scale: float,
    geoXMin: float,
    geoYMin: float,
) -> shapely.Geometry | tuple[float, float]:
    ...
    if returnAsTuple:
        if isinstance(print3DCoord2D, shapely.Point):
            ...
            print("geoCoordToPrint3DCoord: could not return tuple")
...
def plot_DEM_histogram(npim, DEM_name, temp_folder):
    ...
    fig, (ax1, ax2) = plt.subplots(
        nrows=2, figsize=(5, 10), gridspec_kw={"height_ratios": [1, 0.4]}
    )

    npim_flt = npim.flatten()  # 1D array
    elevmin, elevmax = numpy.nanmin(npim_flt), numpy.nanmax(npim_flt)

    imgplot = ax1.imshow(npim, aspect="equal", interpolation="spline36")
    ax1.xaxis.set_ticks([])
    ax1.yaxis.set_ticks([])
    ...
    # plt.show() # DEBUG
    plot_file_name = (
        temp_folder + os.sep + DEM_name + "_elevation_plot_with_histogram.png"
    )
...
def dilate_array(
    raster,
    dilation_source: numpy.ndarray | None = None,
    dilation_cycles: int = 1,
    limit_mask=None,
):
    ...
    if dilation_source is not None:
        ...
        unequal_mask = np.not_equal(
            mask, dilated_mask
        )  # True where the dilated mask is different from the original mask
