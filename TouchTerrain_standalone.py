...
try:
    ...
    print(f"Error: touchterrain module is not installed properly: {e}")
    print("Please run: uv pip install -e . --python .venv/bin/python")
    print("Or if using pip: pip install -e .")
    sys.exit(1)
...
def main():
    ...
    with open("example_config.json", "w+") as fp:
        json.dump(
            args, fp, indent=0, sort_keys=True
        )  # indent = 0: newline after each comma
    print(
    ...
    if len(sys.argv) > 1:  # sys.argv are the CLI args
        ...
        print(f"\n{'='*60}")
        print(f"Reading configuration from: {json_fname}")
        print(f"{'='*60}")
    ...
    if args["importedDEM"] is not None:
        ...
        if not os.path.exists(args["importedDEM"]):
            sys.exit(f"Error: DEM file not found: {args['importedDEM']}")
        file_size_mb = os.path.getsize(args["importedDEM"]) / (1024 * 1024)
        print(f"DEM file size: {file_size_mb:.1f} MB")
    else:
        print(f"\nProcessing Earth Engine DEM: {args['DEM_name']}")
        print(
            f"Area: lat [{args['bllat']:.4f}, {args['trlat']:.4f}], lon [{args['bllon']:.4f}, {args['trlon']:.4f}]"
        )
    ...
    print(
        f"  Tile size: {args['tilewidth']} mm x {args['ntilesx']} x {args['ntilesy']} tiles"
    )
    print(f"  Print resolution: {args['printres']} mm")
    print(f"  Z-scale: {args['zscale']}")
    print(f"  Base thickness: {args['basethick']} mm")
    print(f"  Output format: {args['fileformat']}")
    print(f"{'='*60}\n")

    # Re-enable INFO logging for processing output
    ...
    print(f"\n{'='*60}")
    print(f"SUCCESS! Created zip file: {full_zip_file_name}")
    print(f"Size: {totalsize:.2f} MB")
    ...
    if 0:  # set this to 0 if you don't want the zip file to be unzipped
        ...
        folder = (
            os.getcwd() + os.sep + args["zip_file_name"]
        )  # new stl folder in current folder
