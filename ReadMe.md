...
- `sqrt`: (default: `false`) if true, will apply the square root to the final elevation (after applying z-scale and running _leq operations!) When combined with a very large z-scale (1000 - 5000) this can help to equalize terrain models that have a wide range of elevations, such as from sea level to 2000 m peaks. THis helps to bring out details in low areas while avoiding un-printably pointy mountains. 
